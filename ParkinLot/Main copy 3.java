...
class ParkingLot {
    ...
    private java.util.Map<String, Integer> carLocationMap;

    public ParkingLot(int spotsPerFloor, int numberOfFloors) {
        this.spotsPerFloor = spotsPerFloor;
        this.numberOfFloors = numberOfFloors;
        this.parkedCars = new Car[numberOfFloors][spotsPerFloor];
        this.carLocationMap = new java.util.HashMap<>();
    }

    private int calculateSlotNumber(int floor, int spot) {
        return floor * spotsPerFloor + spot + 1;
    }

    private int getFloorFromSlot(int slotNumber) {
        return (slotNumber - 1) / spotsPerFloor;
    }

    private int getSpotFromSlot(int slotNumber) {
        return (slotNumber - 1) % spotsPerFloor;
    }
        ...
        for (int floor = 0; floor < numberOfFloors; floor++) {
            for (int spot = 0; spot < spotsPerFloor; spot++) {
                if (parkedCars[floor][spot] == null) {
                    ...
                    int slotNumber = calculateSlotNumber(floor, spot);
                    carLocationMap.put(car.getLicensePlate(), slotNumber);
                    System.out.println("Car " + car.getLicensePlate() + " parked at Slot #" + slotNumber + " (Floor " + floor + ", Spot " + spot + ")");
                    return true;
                }
            }
        }
    ...
    public boolean removeCar(String licensePlate) {
        Integer slotNumber = carLocationMap.get(licensePlate);
        if (slotNumber == null) {
            System.out.println("Car " + licensePlate + " not found!");
            return false;
        }
        
        int floor = getFloorFromSlot(slotNumber);
        int spot = getSpotFromSlot(slotNumber);
        
        parkedCars[floor][spot] = null;
        carLocationMap.remove(licensePlate);
        System.out.println("Car " + licensePlate + " removed from Slot #" + slotNumber);
        return true;
    }
