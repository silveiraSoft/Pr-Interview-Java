...
class ParkingLot {
    ...
    private java.util.Map<String, String> carLocationMap;
        ...
        for (int floor = 0; floor < numberOfFloors; floor++) {
            for (int spot = 0; spot < spotsPerFloor; spot++) {
                if (parkedCars[floor][spot] == null) {
                    ...
                    carLocationMap.put(car.getLicensePlate(), "Floor " + floor + ", Spot " + spot);
    ...
    public boolean removeCar(String licensePlate) {
        String location = carLocationMap.get(licensePlate);
        if (location == null) {
        ...
        int spot = Integer.parseInt(parts[1].split(" ")[1]);
        
        parkedCars[floor][spot] = null;
        carLocationMap.remove(licensePlate);
        System.out.println("Car " + licensePlate + " removed from " + location);
        return true;
    }

    public String findCar(String licensePlate) {
        return carLocationMap.getOrDefault(licensePlate, "Car not found");
    }
